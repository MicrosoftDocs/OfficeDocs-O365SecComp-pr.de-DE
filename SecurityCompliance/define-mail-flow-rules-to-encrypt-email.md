...
## <a name="create-mail-flow-rules-to-encrypt-email-messages-with-the-new-ome-capabilities"></a>Erstellen Sie Mail Flow Regeln zum Verschlüsseln von e-Mail-Nachrichten mit den neuen Funktionen von OME
...
### <a name="use-the-eac-to-create-a-rule-for-encrypting-email-messages-with-the-new-ome-capabilities"></a>Verwenden der Exchange-Verwaltungskonsole zum Erstellen einer Regel zum Verschlüsseln von e-Mail-Nachrichten mit den neuen OME-Funktionen
...
5. Wählen Sie in der Liste der e-Mail-Flussregeln, die Regel zu ändern, um die neuen OME-Funktionen verwenden, und wählen Sie dann auf **Bearbeiten** ![Bearbeitungssymbol](media/ebd260e4-3556-4fb0-b0bb-cc489773042c.gif).
...
## <a name="create-mail-flow-rules-for-office-365-message-encryption-without-the-new-capabilities"></a>Erstellen Sie Mail Flow Regeln für Office 365 Message Encryption ohne die neuen Funktionen
...
### <a name="use-the-eac-to-create-a-mail-flow-rule-for-encrypting-email-messages-without-the-new-ome-capabilities"></a>Verwenden der Exchange-Verwaltungskonsole zum Erstellen einer Mail Flow Regel zum Verschlüsseln von e-Mail-Nachrichten, ohne die neuen OME-Funktionen
...
2. Erstellen Sie eine Regel mit dem Cmdlet **New-TransportRule** und der Parameter _ApplyOME_ gibt legen `$true`.
   ...
   **Hinweise**:
   ...
   - Der eindeutige Name der neuen Regel ist "Verschlüsseln Regel für Dr Toni Ramos".

   - Der Parameter _SentTo_ gibt die Empfänger der Nachricht (identifiziert durch Name, e-Mail-Adresse, DN.). In diesem Beispiel wird der Empfänger durch die e-Mail-Adresse "DrToniRamos@hotmail.com" identifiziert.

   - Der Parameter _SentToScope_ gibt den Speicherort der Empfänger der Nachricht. In diesem Beispiel wird das Postfach des Empfängers befindet sich in Hotmail und ist nicht Bestandteil der Office 365-Organisation, sodass der Wert `NotInOrganization` wird verwendet.
...
#### <a name="use-the-eac-to-create-a-rule-for-removing-encryption-from-email-replies-encrypted-without-the-new-ome-capabilities"></a>Verwenden der Exchange-Verwaltungskonsole zum Erstellen einer Regel zum Entfernen der Verschlüsselung aus e-Mail-Antworten verschlüsselt werden, ohne die neuen OME-Funktionen
...
#### <a name="use-exchange-online-powershell-to-create-a-rule-to-remove-encryption-from-email-replies-encrypted-without-the-new-ome-capabilities"></a>Verwenden von Exchange Online PowerShell zum Erstellen einer Regel zum Entfernen der Verschlüsselung aus e-Mail-Antworten verschlüsselt werden, ohne die neuen OME-Funktionen
...
2. Erstellen einer Regel mithilfe des Cmdlets **New-TransportRule** , und legen Sie den Parameter _RemoveOME_ auf `$true`.
   ...
   **Hinweise**:
   ...
   - Der Parameter _SentToScope_ gibt den Speicherort der Empfänger der Nachricht. In diesem Beispiel wird der Wert `InOrganization` Wert verwendet, wodurch angegeben wird: 
     ...
     - E-Mail-Adresse des Empfängers befindet sich in einer akzeptierten Domäne, die als eine autorisierende Domäne oder in Ihrer Organisation, _und_ die Nachricht gesendet oder über eine authentifizierte Verbindung empfangen wurde eine interne Relaydomäne konfiguriert ist.
