...
description: Erweiterte Spam-Filteroptionen bieten Administratoren die Möglichkeit, verschiedene Content Attribute einer Nachricht zu überprüfen. Das Vorhandensein von diese Attribute in einer Nachricht erhöht die Spam-Bewertung der Nachricht (steigert auf das Potenzial für die als Spam identifiziert werden), oder die Nachricht als Spam markiert. Die ASF-Optionen Ziel bestimmte Nachrichteneigenschaften wie HTML-Tags und URL-Umleitung, die häufig in Spam-Nachrichten nicht gefunden werden.
ms.openlocfilehash: fe8b816d5672481dedabd0aa6985d63b13de47e6
ms.sourcegitcommit: 8c5a88433cff23c59b436260808cf3d91b06fdef
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 12/07/2018
ms.locfileid: "27194686"
---
# <a name="advanced-spam-filtering-options"></a>Erweiterte Spamfilterungsoptionen

Erweiterte Spam-Filteroptionen bieten Administratoren die Möglichkeit, verschiedene Content Attribute einer Nachricht zu überprüfen. Das Vorhandensein von diese Attribute in einer Nachricht erhöht die Spam-Bewertung der Nachricht (steigert auf das Potenzial für die als Spam identifiziert werden), oder die Nachricht als Spam markiert. Die ASF-Optionen Ziel bestimmte Nachrichteneigenschaften wie HTML-Tags und URL-Umleitung, die häufig in Spam-Nachrichten nicht gefunden werden.
...
> > [!TIP]
> Optionale Informationen, die einen Benutzer unterstützen attraktiver berücksichtigen aktivieren Ihrer ASF-Optionen, um Spam-Blockierung maximieren im Testmodus werden basierend auf Ihrer Umgebung. Für Kunden mit hohen Spam-Prozentwerten für bestimmte ASF-Optionen wird empfohlen, dass Sie diese Optionen, die sie zuerst in Ihrer produktionsumgebung vor dem Implementieren testen. > Es wird empfohlen, dass Organisationen, die betreffenden Informationen zu Phishing sind Einschalten der **SPF-Datensatz: Schwerer Fehler** Option. 
...
|SPF-Eintrag: Schwerer Fehler  <br/> |Wenn diese Einstellung aktiviert ist, werden Nachrichten, die eine SPF-Prüfung nicht bestehen (d. h. sie wurden von einer nicht im SPF-Eintrag angegebenen IP-Adresse gesendet) als Spam gekennzeichnet. Das Aktivieren dieser Einstellung wird für Organisationen empfohlen, die befürchten, Phishingnachrichten zu erhalten.  <br/> <br/>> [!NOTE]
> Testmodus ist nicht verfügbar für diese Option.  <br/> |X-CustomSpam: SPF-Eintragsfehler  <br/> |
|Bedingte Absender-ID-Filterung: Schwerer Fehler  <br/> |Wenn diese Einstellung aktiviert ist, wird jede Nachricht, für die ein schwerer Fehler bei der bedingten Absender-ID-Filterung auftritt, als Spam gekennzeichnet. Bei dieser Option wird eine SPF-Überprüfung mit einer Überprüfung der Absender-ID kombiniert, um einen Schutz gegen Nachrichtenkopfzeilen zu erreichen, die gefälschte Absender enthalten.  <br/> <br/>> [!NOTE]
> Testmodus ist nicht verfügbar für diese Option.  <br/> |X-CustomSpam: Von-SPF-Eintragsfehler  <br/> |
|NDR-Rückläufer  <br/> |Wenn Sie EOP für den Schutz lokaler Postfächer verwenden, werden bei Aktivierung dieser Einstellung alle legitimen NDR-Nachrichten an den ursprünglichen Absender gesendet, und alle Rückläufernachrichten (nicht legitime NDR-Nachrichten) werden als Spam gekennzeichnet. Wenn Sie diese Einstellung nicht aktivieren, durchlaufen alle NDRs trotzdem die Spamfilterung. In diesem Fall werden die legitimsten Nachrichten an den ursprünglichen Absender gesendet, während einige, aber nicht alle, Rückläufernachrichten als Spam gekennzeichnet werden. Rückläufernachrichten, die nicht als Spam gekennzeichnet sind, werden jedoch nicht an den ursprünglichen Absender gesendet, weil sie an den gefälschten Absender gehen.  <br/> <br/> Wenn Sie den Dienst zum Schutz von cloudgehosteten Exchange Online-Postfächern verwenden, ist es nicht notwendig, diese Einstellung zu konfigurieren.  <br/><br/> > [!NOTE]
> Für beide Szenarien (lokalen und Cloud-Postfächer) ist es auch nicht erforderlich, aktivieren Sie diese Einstellung für ausgehende e-Mails, die über den Dienst gesendet werden, als Unzustellbarkeitsberichte, die legitime Springeffekt Nachrichten werden automatisch erkannt und an den ursprünglichen Absender gesendet werden . > Testmodus ist nicht verfügbar für diese Option.<br/><br/>> [!TIP]
> Tipp: Weitere Informationen zu rückläufernachrichten und EOP finden Sie unter [rückläufernachrichten und EOP](backscatter-messages-and-eop.md).           | X-CustomSpam: RÜCKLÄUFER-NDR<br/> | | Massen-mail  <br/> | Advanced Spam Filtern von Massen-e-Mail-wurde zurückgezogen und durch die schwellenwerteinstellungen Massen und e-Mail ersetzt. Auschecken [Was ist der Unterschied zwischen Junk-e-Mail und Massen-e-Mail?](what-s-the-difference-between-junk-email-and-bulk-email.md) und für Weitere Informationen sowie Informationen zum Konfigurieren der instellungen festlegen [Ihrer Spam-Filter-Richtlinien konfigurieren](configure-your-spam-filter-policies.md) .<br/> ||
