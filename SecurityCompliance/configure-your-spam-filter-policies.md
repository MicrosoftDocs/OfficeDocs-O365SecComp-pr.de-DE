...
4. On the **spam and bulk email actions** page, under **Spam** and **High confidence spam**, select the action to take for incoming spam and bulk email. By default, **Move messages to Junk Email folder** is selected. The other possible values are: 
      ...
      - Sie können den Headertext nicht im Format \< *Header*  \>:\<  *Wert*  \> angeben. Wenn Sie dies tun, werden beide Werte vor und nach dem Doppelpunkt ignoriert, und stattdessen wird der standardmäßige X-Header-Text angezeigt: "X-This-is-Spam: Diese Nachricht ist offenbar Spam." 
...
12. **In** NachRichten werden nach der Regel, die dieser Option zugeordnet ist, aktiv gefiltert. Nachrichten werden entweder als Spam gekennzeichnet, oder ihre Spambewertung wird erhöht, je nach den von Ihnen aktivierten Optionen. 
...
15. Klicken Sie nur für benutzerdefinierte Richtlinien auf das Menüelement **übernehmen auf** , und erstellen Sie dann eine bedingungsbasierte Regel, um die Benutzer, Gruppen und Domänen anzugeben, auf die diese Richtlinie angewendet werden soll. Sie können mehrere Bedingungen angeben, wenn diese eindeutig sind. 
