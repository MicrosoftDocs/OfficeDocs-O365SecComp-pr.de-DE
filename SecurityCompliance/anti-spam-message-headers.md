...
> [!TIP]
>  Sie können den Inhalt der Nachrichtenkopfzeile kopieren und in das Tool für die [Nachrichten Analyse](https://testconnectivity.microsoft.com/?tabid=mha) einfügen. Dieses Tool hilft bei der Analyse der Kopfzeilen und legt Sie in ein mehr readible Format.
...
|SFV:SKN|Die Nachricht wurde bereits als Nicht-Spam markiert, noch bevor sie vom Inhaltsfilter verarbeitet wurde. Dies umfasst Nachrichten, bei denen die Nachricht mit einer Nachrichtenfluss Regel übereinstimmt, um Sie automatisch als nicht-Spam zu kennzeichnen und alle zusätzlichen Filter zu umgehen.|
...
|SFTY|Die Nachricht wurde als Phishing identifiziert und wird auch mit einem der folgenden Werte markiert: <br/>• 9,1: Standardwert. Die Nachricht enthält eine Phishing-URL, kann andere Phishing-Inhalte enthalten oder wurde von einem anderen e-Mail-Filter (beispielsweise einer lokalen Version von Exchange Server) als Phishing markiert, bevor die Nachricht an Office 365 weitergeleitet wird. <br/>• 9,11: fehlerhafte Nachrichten Spoofing-Überprüfungen, bei denen die sendende Domäne im from:-Header identisch mit der oder an der gleichen Organisation wie die empfangende Domäne ausgerichtet ist oder mit ihr übereinstimmt. Dies deutet darauf hin, dass der Nachricht ein Intra-org-Spoofing-Sicherheitstipp hinzugefügt wird. <br/>• 9,19: Nachrichten Fehler bei Domänen Identitätswechsel überprüft, wo die sendende Domäne versucht, die Identität einer Domäne zu imitieren, die dem Empfänger gehört, oder eine benutzerdefinierte Domäne, die durch die Anti-Phishing-Richtlinie geschützt ist. Dies deutet darauf hin, dass der Identitätswechsel-Sicherheitstipp der Nachricht hinzugefügt wird, wenn diese über die Anti-Phishing-Richtlinie aktiviert ist. <br/>• 9,20: Nachrichten Fehler beim Benutzeridentitätswechsel überprüft, wo der sendende Benutzer versucht, die Identität eines Benutzers in der Empfängerorganisation oder einen benutzerdefinierten Benutzer zu imitieren, der durch die Anti-Phishing-Richtlinie geschützt ist. Dies deutet darauf hin, dass der Identitätswechsel-Sicherheitstipp der Nachricht hinzugefügt wird, wenn diese über die Anti-Phishing-Richtlinie aktiviert ist. <br/>• 9,21: fehlerhafte Anti-Spoofing-Überprüfungen durch Nachrichten und die sendende Domäne im from:-Header werden nicht authentifiziert und stammt aus einer externen Domäne. Wird in Kombination mit CompAuth verwendet (siehe Authentifizierungsergebnisse). <br/>• 9,22: identisch mit 9,21, mit dem Unterschied, dass der Benutzer über einen sicheren Absender verfügt, der außer Kraft gesetzt wurde. <br/>• 9,23: identisch mit 9,22, mit der Ausnahme, dass die Organisation über einen zulässigen Absender oder eine zugelassene Domäne verfügt, die außer Kraft gesetzt wurde. <br/>• 9,24: identisch mit 9,23, mit dem Unterschied, dass der Benutzer über eine Exchange-Nachrichtenfluss Regel verfügt, die außer Kraft gesetzt wurde.|
|X-CustomSpam: [ASFOption]|Die Nachricht wurde mit einer erweiterten Spam Filterungs Option abgeglichen. So bedeutet z. B. **X-CustomSpam: Bildlinks zu Remotestandorten**, dass eine Übereinstimmung mit der ASF-Option **Bildlinks zu Remotestandorten** besteht. Informationen dazu, welcher X-Headertext für jede bestimmte ASF-Option hinzugefügt wird, finden Sie unter [Advanced Spam Filtering Options](advanced-spam-filtering-asf-options.md).|
...
|PCL|Der PCL-Wert (Phishing Confidence Level) der Nachricht, der anzeigt, ob es sich um eine Phishing-Nachricht handelt. Dieser Status kann als einer der folgenden numerischen Werte zurückgegeben werden: <br/>• **0-3**: der Inhalt der Nachricht wird wahrscheinlich nicht als Phishing eingestuft. <br/>• **4-8**: der Inhalt der Nachricht wird wahrscheinlich als Phishing eingestuft. <br/>• **-9990**: (nur Exchange Online Schutz) der Inhalt der Nachricht wird wahrscheinlich als Phishing eingestuft.  <br/>  Anhand dieser Werte wird ermittelt, welche Aktion Ihr E-Mail-Client für die Nachrichten ausführt. Outlook verwendet beispielsweise den PCL-Stempel, um den Inhalt verdächtiger Nachrichten zu blockieren. Weitere Informationen zu Phishing und zur Verarbeitung von Phishing-Nachrichten in Outlook finden Sie unter [Aktivieren oder Deaktivieren von Links in e-Mail-Nachrichten](https://support.office.com/article/2D79B907-93B6-4774-82E6-1F0385CF20F8).|
...
|dkim|Beschreibt die Ergebnisse der DKIM-Prüfung für die Nachricht. Mögliche Werte sind:<br/>• **Pass**: gibt die DKIM-Überprüfung für die übergebene Nachricht an. <br/>• **Fail (reason)**: gibt an, dass die DKIM-Überprüfung für die Nachricht fehlgeschlagen ist und warum. Wenn die Nachricht beispielsweise nicht signiert war oder die Signatur nicht überprüft werden konnte. <br/>• **None**: gibt an, dass die Nachricht nicht signiert wurde. Dies kann oder kann nicht darauf hindeuten, dass die Domäne einen DKIM-Eintrag hat oder der DKIM-Eintrag nicht zu einem Ergebnis ausgewertet wird, sondern nur, dass diese Nachricht nicht signiert wurde.|
|Kopfzeile. d|Die in der DKIM-Signatur identifizierte Domäne, sofern vorhanden. Dies ist die Domäne, die für den öffentlichen Schlüssel abgefragt wird.|
|dmarc|Beschreibt die Ergebnisse der DMARC-Prüfung für die Nachricht. Mögliche Werte sind:<br/>• **Pass**: gibt die DMARC-Überprüfung für die übergebene Nachricht an. <br/>• **Fail**: gibt an, dass die DMARC-Überprüfung für die Nachricht fehlgeschlagen ist. <br/>• **bestguesspass**: gibt an, dass kein DMARC TXT-Eintrag für die Domäne vorhanden ist, aber wenn eine vorhanden war, würde die DMARC-Überprüfung für die Nachricht übergeben haben. Dies liegt daran, dass die Domäne in der 5321. MailFrom-Adresse mit der Domäne in der 5322. from-Adresse übereinstimmt. <br/>• **None**: gibt an, dass kein DKIM TXT-Eintrag für die sendende Domäne in DNS vorhanden ist.|
|action|Gibt die Aktion an, die vom Spamfilter basierend auf den Ergebnissen der DMARC-Prüfung ausgeführt wird. Beispiel:<br/>• **PermError**: während der DMARC-Auswertung ist ein dauerhafter Fehler aufgetreten, beispielsweise beim Auftreten eines falsch formatierten DMARC txt-Eintrags in DNS. Der Versuch, die Nachricht erneut zu senden, wird höchstwahrscheinlich kein anderes Ergebnis liefern. Stattdessen müssen Sie sich möglicherweise an den Domänenbesitzer wenden, um das Problem zu beheben. <br/>• **TempError**: während der DMARC-Auswertung ist ein temporärer Fehler aufgetreten. Sie können den Absender der Nachricht möglicherweise bitten, die Nachricht später noch einmal erneut zu senden, damit sie dann ordnungsgemäß verarbeitet werden kann. <br/>• **oreject** oder **o. Reject**: steht für override Reject. Office 365 wendet diese Aktion an, wenn es Nachrichten empfängt, die die DMARC-Prüfung nicht bestehen und die von Domänen stammen, für deren DMARC-TXT-Eintrag die Richtlinie „p=reject“ festgelegt ist. Anstatt die Nachricht abzulehnen oder zu löschen, kennzeichnet Office 365 die Nachricht als Spam. Weitere Informationen darüber, warum Office 365 auf diese Weise konfiguriert ist, finden Sie unter [So behandelt Office 365 eingehende E-Mail-Nachrichten, die DMARC-Prüfungen nicht bestehen](use-dmarc-to-validate-email.md#inbounddmarcfail). <br/>• **PCT. Quarantine**: gibt an, dass ein Prozentsatz von weniger als 100% der Nachrichten, die DMARC nicht übergeben, trotzdem zugestellt wird. Dies bedeutet, dass die Nachricht nicht erfolgreich DMARC und die Richtlinie auf Quarantäne festgelegt wurde, das PCT-Feld jedoch nicht auf 100% festgelegt wurde und das System nach dem Zufallsprinzip ermittelt, dass die DMARC-Aktion gemäß der angegebenen Domänenrichtlinie nicht angewendet werden soll. <br/>• **PCT. Reject**: gibt an, dass ein Prozentsatz von weniger als 100% der Nachrichten, die DMARC nicht übergeben, trotzdem zugestellt wird. Dies bedeutet, dass die Nachricht nicht erfolgreich DMARC und die Richtlinie auf ablehnen festgelegt wurde, das Feld PCT jedoch nicht auf 100% festgelegt wurde und das System nach dem Zufallsprinzip ermittelt, dass die DMARC-Aktion gemäß der angegebenen Domänenrichtlinie nicht angewendet werden soll.|
|Kopfzeile. from|Die Domäne der „Von“-Adresse in der Kopfzeile der E-Mail-Nachricht. Dies wird manchmal als _5322. from_ -Adresse bezeichnet.|
|compauth|Ergebnis der zusammengesetzten Authentifizierung. Wird von Office 365 verwendet, um mehrere Authentifizierungstypen wie SPF, DKIM, DMARC oder andere Teile der Nachricht zu kombinieren, um zu bestimmen, ob die Nachricht authentifiziert wurde oder nicht. Verwendet die from:-Domäne als Grundlage für die Auswertung.|
|Grund|Der Grund, an dem die kombinierte Authentifizierung erfolgreich war oder fehlgeschlagen ist. Der Wert für den Grund besteht aus drei Ziffern: <br/>• **000**: die Nachricht hat die Authentifizierung explizit nicht erfolgreich ausgeführt. Beispielsweise hat die Nachricht einen DMARC-Fehler mit einer Aktion von Quarantine oder Reject erhalten. <br/>• **001**: die Nachricht hat die Authentifizierung implizit nicht ausgeführt, und die sendende Domäne hat keine Authentifizierungsrichtlinien veröffentlicht. Beispielsweise eine DMARC-Richtlinie von p = None. <br/>• **1xx**: die Nachricht hat die Authentifizierung übergeben. Die zweiten beiden Ziffern sind interne Codes, die von Office 365 verwendet werden. <br/>• **2xx**: die Nachricht mit Soft-bestandenen Authentifizierung. Die zweiten beiden Ziffern sind interne Codes, die von Office 365 verwendet werden. <br/>• **3xx**: die Nachricht wurde nicht auf die kombinierte Authentifizierung überprüft. <br/>• **4xx**: die Nachricht hat die kombinierte Authentifizierung umgangen. Die zweiten beiden Ziffern sind interne Codes, die von Office 365 verwendet werden.|
